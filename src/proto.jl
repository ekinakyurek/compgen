...
function viz(model::ProtoVAE, data; N=5, beam=true)
    ...
    for i=1:length(samples)
        ...
        println("----------------------")
        attension_visualize2(model.vocab, probs, scores, xp, x; prefix="$i")
    end
end
...
function attension_visualize2(vocab, probs, scores, xp, x; prefix="")
...
function process_for_viz2(vocab, pred, xp, I, D, scores, probs)
    ...
    for i = 1 : (N ÷ B) + 1
        ...
        if (d = getbatch(model,dt,b)) !== nothing
            ...
            if beam
                ...
                if forviz
                    for i=1:b
                        @inbounds push!(samples, process_for_viz2(vocab,
                        preds[1][i,:],
                        xp_masked[i,:],
                        ID.I[i,:],
                        ID.D[i,:],
